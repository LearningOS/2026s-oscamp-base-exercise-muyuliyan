...
impl Scheduler {
    ...
    fn schedule_next(&mut self) {
        ...
        // find next ready thread in round-robin order, starting from current + 1
        let next = (old + 1..self.threads.len())
            .chain(0..=old)
            .find(|&i| self.threads[i].state == ThreadState::Ready);

        // If no other thread is ready, just return to let current thread continue running.
        let Some(next) = next else {
            return;
        };

        // If current thread is not finished, mark it as ready again.
        ...
        // If the next thread has an entry, set it to `CURRENT_THREAD_ENTRY` for the wrapper to call and clear it from the thread struct.
