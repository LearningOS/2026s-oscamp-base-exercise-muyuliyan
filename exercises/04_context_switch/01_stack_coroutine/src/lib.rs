...
impl TaskContext {
    ...
    pub fn init(&mut self, stack_top: usize, entry: usize) {
        ...
        self.sp = (stack_top as u64) & !0xf; // align down
    }
}
...
pub unsafe extern "C" fn switch_context(_old: *mut TaskContext, _new: *const TaskContext) {
    // Arguments follow riscv64 ABI: a0 = old, a1 = new.
    core::arch::naked_asm!(
        ...
        // Zero a0/a1 to avoid leaking pointers into the new context
        "mv a0, zero",
        "mv a1, zero",

        // Return to new context (jumps to new.ra)
        "ret",
    )
}
