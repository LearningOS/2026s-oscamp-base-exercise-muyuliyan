...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let pte2 = self.nodes[&self.root_ppn].entries[vpn2];
        let node1_ppn = if pte2 & PTE_V == 0 {
            let new_ppn = self.alloc_node();
            self.nodes
                .get_mut(&self.root_ppn)
                .expect("root page table must exist")
                .entries[vpn2] = (new_ppn << PPN_SHIFT) | PTE_V;
            new_ppn
        } else {
            pte2 >> PPN_SHIFT
        };
        ...
        let pte1 = self
            .nodes
            .get(&node1_ppn)
            .expect("level-1 page table must exist")
            .entries[vpn1];
        let node0_ppn = if pte1 & PTE_V == 0 {
            let new_ppn = self.alloc_node();
            self.nodes
                .get_mut(&node1_ppn)
                .expect("level-1 page table must exist")
                .entries[vpn1] = (new_ppn << PPN_SHIFT) | PTE_V;
            new_ppn
        } else {
            pte1 >> PPN_SHIFT
        };
        ...
        self.nodes
            .get_mut(&node0_ppn)
            .expect("level-0 page table must exist")
            .entries[vpn0] = ((pa >> 12) << PPN_SHIFT) | flags;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        let pte2 = match self.nodes.get(&self.root_ppn) {
            Some(node2) => node2.entries[vpn2],
        ...
        if pte2 & (PTE_R | PTE_W | PTE_X) != 0 {
            // 1GiB leaf at level 2: keep low 30 bits as page offset.
            let ppn = pte2 >> PPN_SHIFT;
            let offset = va & ((1u64 << 30) - 1);
            return TranslateResult::Ok((ppn << 12) | offset);
        }

        let node1_ppn = pte2 >> PPN_SHIFT;
        let pte1 = match self.nodes.get(&node1_ppn) {
            Some(node1) => node1.entries[vpn1],
            None => return TranslateResult::PageFault,
        };
        if pte1 & PTE_V == 0 {
            return TranslateResult::PageFault;
        ...
        if pte1 & (PTE_R | PTE_W | PTE_X) != 0 {
            // 2MiB leaf at level 1: keep low 21 bits as page offset.
            let ppn = pte1 >> PPN_SHIFT;
            let offset = va & ((1u64 << 21) - 1);
            return TranslateResult::Ok((ppn << 12) | offset);
        }

        let node0_ppn = pte1 >> PPN_SHIFT;
        let pte0 = match self.nodes.get(&node0_ppn) {
            Some(node0) => node0.entries[vpn0],
            None => return TranslateResult::PageFault,
        };
        ...
        let ppn = pte0 >> PPN_SHIFT;
        let offset = va & ((1u64 << 12) - 1);
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        self.nodes
            .get_mut(&node1_ppn)
            .expect("level-1 page table must exist")
            .entries[vpn1] = ((pa >> 12) << PPN_SHIFT) | flags;
    }
}
